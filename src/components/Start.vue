...
// Firestore refs
const surveyCollectionRef = collection(db, "Bruz");
const counterDocRef = doc(db, "counterBruz", "surveyCounter");
...
const startMessage = computed(() => {
  if (answers.value.Q1 === 1) {
    return "Bonjour, dans le cadre d'une étude sur les habitudes de fréquentation du cinéma \"CINEVILLE\" Rennes Métropole souhaiterait en savoir plus sur votre déplacement en cours. Acceptez-vous de répondre à quelques questions ?";
  } else if (answers.value.Q1 === 2) {
    return "Bonjour, pour mieux connaître les usagers de la Gare de Bruz Rennes Métropole souhaiterait en savoir plus sur votre déplacement en cours. Acceptez-vous de répondre à quelques questions ?";
  }
  return "";
});
...
const getNextId = async () => {
  ...
  return `Bruz-${counter.toString().padStart(6, "0")}`;
};
</script>
